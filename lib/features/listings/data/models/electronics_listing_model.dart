class ElectronicsListingModel {
  ...
  String get subcategoryLabel {
    const map = {
      'tvs-video-audio': 'TVs, Video-Audio',
      'kitchen-other-appliance': 'Kitchen & Other Appliance',
      'fridges': 'Fridges',
  ...
  factory ElectronicsListingModel.fromMap(Map<String, dynamic> map) {
    final cd = (map['category_data'] as Map<String, dynamic>?) ?? {};
    final imgs = (map['images'] as List<dynamic>?)
            ?.map((e) => e.toString())
            .toList() ??
        [];
    return ElectronicsListingModel(
      id: map['id']?.toString() ?? '',
